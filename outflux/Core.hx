...
class Core {
  ...
  future<T>():Future<T> {
    return new outflux.core.FutureImpl();
  }
  ...
  part<S,B,G,E>(parent:Dynamic,?info:PartInfo):Part_<S,B,G,E> {
    return new outflux.core.PartBaseImpl(parent,info);
  }

  public static function cancelledFuture() {
    return outflux.core.FutureImpl.dead();
  }
  
  public static inline function
  event<T>():Observable<T> {
    return new outflux.core.ObservableImpl();
  }
  ...
  public static function
  listParts() {
    outflux.core.PartBaseImpl.runningParts.foreach(function(p) {
        if (p.info() != null) {
          trace(p.info());
        }
      });    
