...
import outflux.sys.WriteStreamImpl;
import outflux.sys.ReadStreamImpl;
...
class Sys {
  ...
  static var _events = new outflux.sys.Events();
 ...
 public static function umask(?m:Int):Int {
  ...
  spawn(command: String,?args: Array<String>,?options: Dynamic ) : Outcome<String,SysChildProcess> {
    if (args == null)
      args =[];
    return outflux.sys.ChildProcessImpl.spawn(command,args,options);
  }

  public static function
  exec(command: String,?options:Dynamic,?cb:SysChildProcess->Void):Outcome<SysChildExit,String>{
    return outflux.sys.ChildProcessImpl.exec(command,options,cb);
  }

  public static function
  execFile(command: String,?options:Dynamic,?cb:SysChildProcess->Void):Outcome<SysChildExit,String> {
    return outflux.sys.ChildProcessImpl.execFile(command,options,cb);
  }
