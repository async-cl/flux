...
import outflux.data.RemoteBucketProxy;
import outflux.data.RemoteHashProxy;
...
class Data {
  public static function store<T>(kind:StoreKind):Outcome<String,Store> {
    ...
    switch(kind) {
    case SQLITE(name):
      new outflux.data.Sqlite3Store(p,name);
    case REMOTESQLITE(url):
      new outflux.data.RemoteSqlite3Client(p,url);
    }
    return p;
  }
